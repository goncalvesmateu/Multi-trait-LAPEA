...
- [Correlated response to selection]{style="color:red"}: selection for one trait will cause a correlated response in the other (if genetic correlation exists). The expected response of a trait Y, when selection is applied to another trait X:
...
DT <- DT_cpdata
head(DT)
# Removing rows with NAS because is preventing correlation in corss validation 
DT <- na.omit(DT)
# Marker data
GT <- GT_cpdata
GT_cpdata[1:5,1:5]
...
A <- A.mat(GT)
colnames(A) <- rownames(A) <- rownames(DT)
A[1:5,1:5]
