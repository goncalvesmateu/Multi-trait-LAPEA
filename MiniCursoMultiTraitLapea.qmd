...
## Correlation between traits {style="font-size: 80%;"}
...
::: footer
[Reference @falconer1996introduction]
:::
## Genetic correlation
...
$$ Rx = {k}_{p} {h}_{x}{\sigma_a}x$$
$$Ry = {k}_{p} {h}_{y}{\sigma_a}y$$

${k}_{p}$: is the selection differential when the proportion of selected individuals is $p$;<br> ${h}$: is the accuracy of individual selection (direct selection);<br> 
...
![**Fig 2**. *Yield of maize in tropical environment*](Images\cornTrop.jpg){fig-align="center" width=80%}


![**Fig 3**. *Yield of maize in temparate environment*](Images\cornTemp.png){fig-align="center" width=80%}
:::

## Correlated response to selection

- [Correlated response to selection]{style="color:red"}: selection for one trait will cause a correlated response in the other (if genetic correlation exists). The expected response for a trait Y, when selection is applied to another trait X:

$$CR_{Y} = {k}_{p} {h}_{x} {h}_{y}{r}_{g}\sigma_{p_{Y}}$$

- [Indirect selection is expected to be better than direct selection if the secondary trait has substantially higher heritability, and the genetic correlation is high.]{style="color:red"}
...
A certain level of merit (minimun level of performance) for individuals is established for each trait, and all individuals below that level are discarded regardless of values for other traits.
...
\begin{bmatrix}
   ...
   ,
   \begin{bmatrix}
   \sigma_{g}^2 \mathbf{K} & 0 \\
   0 & \sigma_{e}^2 \mathbf{R} \\
    \end{bmatrix}
   \end{pmatrix}
\end{align}
...
\begin{gather*}
y_{i} = X_{i}b_{i} + Z_{i}u_{i} + e_{i} , & i = 1, 2,.. trait
\end{gather*}
...
\begin{bmatrix}
   ...
   \begin{pmatrix}
   \begin{bmatrix}
   \mathbf{G}\otimes\mathbf{K} & 0 \\
   0 &  \mathbf{\Sigma}\otimes{I} \\
    \end{bmatrix}
   \end{pmatrix}
\end{align}
...
\begin{align}
  G =
  \left[ {\begin{array}{cc}
    \sigma_{g_{11}}^2 & \sigma_{g_{12}}^2 \\
    \sigma_{g_{21}}^2 & \sigma_{g_{22}}^2 \\
  \end{array} } \right]
\end{align}
:::
...
\begin{align}
  \Sigma =
  \left[ {\begin{array}{cc}
    \sigma_{e_{11}}^2 & \sigma_{e_{12}}^2 \\
    \sigma_{e_{21}}^2 & \sigma_{e_{22}}^2 \\
  \end{array} } \right]
\end{align}
:::
...
- The prediction accuracy of low-heritability, difficult, and/or expensive to measure traits can be increased by using multi-trait models when the degree of correlation between traits is at least moderate,  to improve the target trait or all the correlated traits simultaneously [@calus2011accuracy]. 

- Multi-trait models can be useful for increasing prediction accuracy when the traits of interest are not measured in all individuals of the testing set, but were observed in individuals in the training set [@jia2012multiple].
...
- [Sommer package](https://github.com/covaruber/sommer)

- [Chapter 5](https://link.springer.com/chapter/10.1007/978-3-030-89010-0_5#Abs1)

![[@montesinos2022multivariate]](Images\bookmulti.jpg){width=50% preview-link="true"}
:::
